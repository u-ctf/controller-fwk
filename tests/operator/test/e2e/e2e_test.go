...
var _ = Describe("Manager", Ordered, func() {
	...
	Context("Resource", func() {
		...
		var configMapName string = uuid.NewString()
		var configMapData = map[string]string{
			"coucou": "cm",
		}

		It("should be creatable in the cluster", func() {
			...
			Expect(err).NotTo(HaveOccurred(), "Create the custom resource")
			...
			verifyCm := func(g Gomega) {
				...
				g.Expect(err).NotTo(HaveOccurred(), "Get the generated config map")
				g.Expect(cm.Data).To(Equal(configMapData), "Config map data mismatch")
			}
			Eventually(verifyCm, 5*time.Minute).Should(Succeed())

		})

		It("should be deletable from the cluster", func() {
			By("deleting the test resource")
			err := c.Delete(ctx, &testResource)
			Expect(err).NotTo(HaveOccurred(), "Delete the custom resource")
			...
			verifyCm := func(g Gomega) {
				err = c.Get(ctx, client.ObjectKeyFromObject(cm), cm)
				g.Expect(err).To(HaveOccurred(), "Config map should be deleted")
				g.Expect(client.IgnoreNotFound(err)).To(Succeed(), "Config map should be deleted")
			}
			Eventually(verifyCm, 5*time.Minute).Should(Succeed())
		})
